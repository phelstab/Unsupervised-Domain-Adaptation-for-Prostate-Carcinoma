...
def prepare_datagens(args, fold_id):
    ...
    # load paths to images and labels - select balanced subset with positives
    train_labels = np.array(train_json['case_label'])
    valid_labels = np.array(valid_json['case_label'])
    
    # Get indices for positive and negative cases
    train_pos_idx = np.where(train_labels == 1)[0][:15]  # first 5 positive cases
    train_neg_idx = np.where(train_labels == 0)[0][:15]  # first 5 negative cases
    valid_pos_idx = np.where(valid_labels == 1)[0][:15]  # first 5 positive cases
    valid_neg_idx = np.where(valid_labels == 0)[0][:15]  # first 5 negative cases
    
    # Combine indices
    train_indices = np.concatenate([train_pos_idx, train_neg_idx])
    valid_indices = np.concatenate([valid_pos_idx, valid_neg_idx])
    
    # Select balanced subset
    train_data = [np.array(train_json['image_paths'])[train_indices], np.array(train_json['label_paths'])[train_indices]]
    valid_data = [np.array(valid_json['image_paths'])[valid_indices], np.array(valid_json['label_paths'])[valid_indices]]
    
    print(f">>> USING LIMITED DATASET: {len(train_data[0])} TRAINING SAMPLES, {len(valid_data[0])} VALIDATION SAMPLES <<<")
    print(f">>> THIS IS FOR OVERFITTING TEST ONLY! <<<")
