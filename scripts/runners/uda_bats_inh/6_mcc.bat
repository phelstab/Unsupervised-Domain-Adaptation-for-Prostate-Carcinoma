@echo off
:: MCC UDA Experiments for merged public source -> UULM target
cd /d "%~dp0..\..\.."
...
set "COMMON=--da-method mcc --binary --backbone %BACKBONE% --plot-oracle --checkpoint-interval 2 --no-early-stopping --two-splits-source --target-cv-folds 3 --class-weights %LR_SCHEDULER% %SPLIT% %UULM_META% --aug-all"

echo ========================================================
echo MCC UDA Experiments (RUMC+PCNN+ZGT -> UULM)
