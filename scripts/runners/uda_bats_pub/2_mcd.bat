@echo off
:: MCD UDA Experiments for public dataset splits (2-split mode)
cd /d "%~dp0..\..\.."
...
set "COMMON=--da-method mcd --binary --backbone %BACKBONE% --plot-oracle --checkpoint-interval 2 --no-early-stopping --two-splits-source --two-splits-target --target-cv-folds 3 --class-weights %LR_SCHEDULER% --aug-all"

echo ========================================================
echo MCD UDA Experiments (Public Splits 2)
