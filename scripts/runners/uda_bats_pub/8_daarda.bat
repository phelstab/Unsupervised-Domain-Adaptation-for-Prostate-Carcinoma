@echo off
:: DAARDA UDA Experiments for public dataset splits (2-split mode)
cd /d "%~dp0..\..\.."
...
set "COMMON=--da-method daarda --binary --backbone %BACKBONE% --plot-oracle --checkpoint-interval 2 --no-early-stopping --two-splits-source --two-splits-target --target-cv-folds 3 --class-weights %LR_SCHEDULER% --aug-all %DAARDA_ARGS%"

echo ========================================================
echo DAARDA UDA Experiments (Public Splits 2)
echo Split: RUMC_to_PCNN, RUMC_to_ZGT (2-split mode)
echo Full regularization with class weights
echo DAARDA settings: divergence=js_beta, relax=1.5, grad_penalty=0.0
